<div class="space-y-4">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold">{{ __('My Class Students') }}</h1>
            <p class="text-zinc-600 dark:text-zinc-400">{{ __('Manage students enrolled in your classes') }}</p>
        </div>
    </div>
                ...
                @forelse ($this->students as $student)
                    <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800">
                        ...
                        <td class="px-4 py-3">
                            @php
                                $enrollments = $student->enrollments()
                                    ->whereHas('course', fn($q) => $q->where('faculty_profile_id', auth()->user()->facultyProfile->id))
                                    ->get();
                                $statuses = $enrollments->pluck('status')->unique()->implode(', ');
                            @endphp
                        ...
                        <td class="px-4 py-3 text-center">
                            <flux:button
                                href="{{ route('students.show-profile', $student) }}"
                                variant="ghost"
                                size="sm"
                                icon="arrow-right"
                                wire:navigate
